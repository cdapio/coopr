...
# Add Coopr embedded bin PATH, if it exists
if [ -d /opt/coopr/embedded/bin ] ; then
    export PATH=/opt/coopr/embedded/bin:${PATH}
fi
...
function request_superadmin_workers () {
    ...
    curl --silent --request PUT \
      ...
      --header "Coopr-UserID:${COOPR_API_USER}" \
      --header "Coopr-TenantID:${COOPR_TENANT}" \
      --connect-timeout 5 --data "{\"workers\":${COOPR_NUM_WORKERS}, \"name\":\"superadmin\"}" \
...
function provisioner () {
    ...
    then
        $COOPR_HOME/bin/dummy-provisioner.sh $1 
    else
        $COOPR_HOME/bin/provisioner.sh $1
    fi
}
...
case "$1" in
  start)
    $COOPR_HOME/bin/server.sh start && \
    $COOPR_HOME/bin/ui.sh start && \
    load_defaults && \
    provisioner start && \
  ...
  stop)
    provisioner stop
    $COOPR_HOME/bin/server.sh stop
    $COOPR_HOME/bin/ui.sh stop
  ;;
  ...
  status)
    $COOPR_HOME/bin/server.sh status
    $COOPR_HOME/bin/ui.sh status
    provisioner status
  ;;

...
function createToggles($scope) {
  ...
  $scope.toggleFilters = function() {
    angular.forEach($scope.columns, function(column) {
      column.filterVisible = shouldFilterBeVisibleWhenToggleFilters(column);
    });
    $scope.globalFilterVisibleState = !$scope.globalFilterVisibleState;
    $scope.inputGlobalSearch = "";
    if (angular.isDefined($scope.tableParams)) {
      $scope.tableParams.reload();
    }
...
function createTable($scope, nodes, ngTableParams, $filter, defaultColumns) {
  ...
  $scope.tableParams = new ngTableParams({
    ...
    getData: function($defer, params) {
      ...
      if ($scope.globalFilterVisibleState) {
        filteredData = params.filter() ? $filter("regex")(filteredData, params.filter(), $scope) : filteredData;
      } else {
...
function shouldFilterBeVisibleWhenToggleColumn(column, globalFilterVisibleState) {
  return shouldFilterBeVisible(column.visible, globalFilterVisibleState);
}

function shouldFilterBeVisibleWhenToggleFilters(column) {
  return shouldFilterBeVisible(column.visible, !column.filterVisible);
}

function shouldFilterBeVisible(columnVisibleState, filterVisibleState) {
  var result = false;
  if (filterVisibleState === false && columnVisibleState === false) {
    result = false;
  } else if (filterVisibleState === false && columnVisibleState === true) {
    result = false;
  } else if (filterVisibleState === true && columnVisibleState === false) {
    result = false;
  } else if (filterVisibleState === true && columnVisibleState === true) {
    result = true;
  }

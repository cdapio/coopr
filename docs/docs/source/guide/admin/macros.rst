...
hostnames or IP addresses of other nodes within a cluster. For this reason, Cask Coopr supports several macros that allow administrators
...
Two macro functions are also supported in Cask Coopr: join and map.
::
...
nodes. A user creates a cluster from this template, and Cask Coopr decides to place the namenode and
...
Cask Coopr will see the macro ``%host.service.hadoop-hdfs-namenode%`` and replace it with a comma separated list of all the hosts

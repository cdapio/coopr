...
var COOPR_HOME = process.env.COOPR_HOME || (__dirname + '/../'),
  ...
  SERVER_PROTO = COOPR_SSL ? 'https' : 'http',
  ...
  COOPR_UI_VERIFY_CERT = ('true' === process.env.COOPR_UI_VERIFY_CERT),
  COOPR_NODE_TLS_ENABLED = ('true' === process.env.COOPR_NODE_TLS_ENABLED),
  COOPR_UI_TLS_KEY_FILE = process.env.COOPR_UI_TLS_KEY_FILE,
  COOPR_UI_TLS_KEY_PASS = process.env.COOPR_UI_TLS_KEY_PASS,
  COOPR_UI_TLS_CRT_FILE = process.env.COOPR_UI_TLS_CRT_FILE,
  COOPR_UI_TLS_CA_FILE = process.env.COOPR_UI_TLS_CA_FILE,
  COOPR_SERVER_URI = process.env.COOPR_SERVER_URI || SERVER_PROTO + '://127.0.0.1:55054';
...
var pkg = require('./package.json'),
  ...
  proxy = null,
  proxySecureObject = {key: '', cert: '', ca: '', passphrase: ''},
...
if (COOPR_NODE_TLS_ENABLED) {
  try {
    proxySecureObject.ca = fs.readFileSync(COOPR_UI_TLS_CA_FILE, 'utf-8');
  } catch (e) {
    console.log('Can not read TLS CA file:', COOPR_UI_TLS_CA_FILE);
  }

  try {
    proxySecureObject.cert = fs.readFileSync(COOPR_UI_TLS_CRT_FILE, 'utf-8');
  } catch (e) {
    console.log('Can not read TLS CRT file:', COOPR_UI_TLS_CRT_FILE);
  }

  try {
    proxySecureObject.key = fs.readFileSync(COOPR_UI_TLS_KEY_FILE, 'utf-8');
  } catch (e) {
    console.log('Can not read TLS KEY file:', COOPR_UI_TLS_KEY_FILE);
  }

  proxySecureObject.passphrase = COOPR_UI_TLS_KEY_PASS;
}

proxy = httpProxy.createProxyServer({
  target: COOPR_SERVER_URI,
  ssl: COOPR_NODE_TLS_ENABLED ? proxySecureObject : null,
  secure: COOPR_UI_VERIFY_CERT
});
...
try {
  app.use(serveFavicon(__dirname + '/dist/assets/img/favicon.png'));
}
catch (e) {
  console.error("Favicon missing! Did you run `gulp build`?");
}

...
/**
 * Restarts service on cluster.
 */
public class RestartServiceOnClusterCommand extends AbstractAuthCommand {

  private final ClusterClient clusterClient;

  @Inject
  private RestartServiceOnClusterCommand(ClusterClient clusterClient, CLIConfig cliConfig) {
    super(cliConfig);
  ...
  public void perform(Arguments arguments, PrintStream printStream) throws Exception {
    ...
    if (clusterOperationRequest == null) {
      clusterClient.restartServiceOnCluster(clusterId, serviceId);
    } else {
      clusterClient.restartServiceOnCluster(clusterId, serviceId, clusterOperationRequest);
    }
  }

  @Override
  public String getPattern() {
    return String.format("restart service <%s> on cluster <%s> [with provider fields <%s>]",

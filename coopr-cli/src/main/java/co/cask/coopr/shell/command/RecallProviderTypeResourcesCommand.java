...
public class RecallProviderTypeResourcesCommand implements Command {
  ...
  public void execute(Arguments arguments, PrintStream printStream) throws Exception {
    ...
    pluginClient.recallProviderTypeResource(automatorTypeId, resourceType, resourceName, version);
  }

  @Override
  public String getPattern() {
    return String.format("recall resource-type <%s> with name <%s> version <%s> from provider <%s>",
                         RESOURCE_TYPE, RESOURCE_NAME, RESOURCE_VERSION, AUTOMATOR_TYPE_ID);
  }

  @Override
  public String getDescription() {
    return "Recall provider type resource version.";
  }
}
